class Solution {
    public List<List<String>> accountsMerge(List<List<String>> accounts) {
        ...
        Map<String, Set<String>> graph = new HashMap<>(); //把email都连起来，有共同的则可以通过DFS找到
        ...
        for (String email : graph.keySet()) {
            if (!visited.contains(email)) { //a new entry
                List<String> cur = new LinkedList<>();//add 0 index is O(1)
    ...
    public void dfs(Map<String, Set<String>> graph, List<String> cur, Set<String> visited, String email) {
        visited.add(email);
        cur.add(email);
