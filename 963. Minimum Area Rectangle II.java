    ...
    public double minAreaFreeRect(int[][] points) {
        ...
        for (int i = 0; i < len; i++) {
            for (int j = i + 1; j < len; j++) {
                ...
                if (map.get(key) == null) map.put(key, new ArrayList<int[]>()); //对角线长度相等且中心点相等就是矩形
            ...
            if (map.get(key).size() > 1) {  
                ...
                for (int i = 0; i < list.size(); i++) { // there could be multiple rectangles inside
                    for (int j = i + 1; j < list.size(); j++) {
                        ...
                        double len1 = Math.sqrt((points[p1][0] - points[p2][0]) * (points[p1][0] - points[p2][0]) +  (points[p1][1] - points[p2][1]) * (points[p1][1] - points[p2][1])); //两点距离公式
