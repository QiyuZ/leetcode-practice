    ...
    public boolean possibleToStamp(int[][] grid, int stampHeight, int stampWidth) {
        ...
        for (int r = 0; r < height; r++) {
            for (int c = 0; c < width; c++) {
                if (grid[r][c] != FILLED) {
                    grid[r][c] = pre[c] == 1 ? -1: Math.max(pre[c] - 1, -stampHeight); //mark the place can be covered
                }
            }
            pre = grid[r];
        }
        ...
        for (int r = stampHeight - 1; r < height; r++) { 
            ...
            for (int c = 0; c < width; c++) {
                ...
                if (cnt == stampWidth) fill(grid, r, c, stampHeight, stampWidth); //means we can cover here
